...
function getMachine (){
    $.ajax({
        url:"http://192.9.132.128:8080/api/Machine/all",
        type:"GET",
        datatype:"JSON",
...
function postMachine(){
    ...
    console.log(cajas);
    $.ajax({
        url:"http://192.9.132.128:8080/api/Machine/save",
        type:"POST",
        datatype:"JSON",
...
function putMachine(idDesdeBoton){
    ...
    $.ajax({
        url:"http://192.9.132.128:8080/api/Category/update",
        type:"PUT",
        datatype:"JSON",
...
function deleteMachine(idDesdeBoton){
    ...
    $.ajax({
        url:"http://192.9.132.128:8080/api/Category/"+idDesdeBoton,
        type:"DELETE",
        datatype:"JSON",
...
function pintarMachine(json_maquinas){
    ...
    for(i=0;i<json_maquinas.length;i++){
        ...
        myTable+="<td> <button class='flex mx-auto text-white bg-indigo-500 border-0 py-2 px-8 focus:outline-none hover:bg-indigo-600 rounded text-lg' onclick='putCategoria("+json_maquinas[i].name+") '> Actualizar</button>"
        myTable+="<td> <button class='flex mx-auto text-white bg-indigo-500 border-0 py-2 px-8 focus:outline-none hover:bg-indigo-600 rounded text-lg' onclick='deleteCategoria("+json_maquinas[i].name+")'> Borrar</button>"
        myTable+="</tr>";
    }
    myTable+="</table>";
...
function getCategoria_Machine(){
    $.ajax({
        url:"http://192.9.132.128:8080/api/Category/all",
        type:"GET",
        datatype:"JSON",
        success:function(respuesta){
